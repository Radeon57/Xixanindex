...
- **Power display scale.** Stats grow exponentially, so every *power* number the player sees (attack, defence, HP, clone and monster power, ค่ายุทธ์, god and ultimate stats, tribulation damage) goes through ui.js `pw()` = `POW_C * value^POW_K` (data.js, now 10·√x). Multipliers on those stats show as `pwM(f)`/`statM(stat, f)`, HP and damage as `pwPool(cur, max)`, and data.js stat-bonus descs use `px()`/`pp()`/`pa()`. The engine and balance always use the real values. พลังเทวะ (DP), ปราณเทพ, speed and clone counts are not scaled.
- **Balance.** A fresh-save bot kills gods 1–6 at about 3.5 / 9 / 16 / 25 / 36 / 48 minutes (`first god within ~4 min` keeps the opening fun). Late-save idle-bot timings were about 3.4 / 4.8 / 6.6 / 9.1 hours for gods 7–10. Re-check them if you change numbers in `data.js`.
- **God mechanics.** Each god has one deterministic `mech` (data.js `GOD_MECHS`: charge, regen, revive, miss, guard, burn, rage, reflect). `stepFight` applies it per exchange through `mechDealt`/`mechTaken`, and `fightHits` predicts the same fight in closed form for `outlook` and `neededFactor`. Keep the two in step: `tests/engine_fuzz.js` section G fails if any forecast differs from the real fight. God stats were lowered by each mechanic's measured difficulty so kill times did not move; re-measure if you change a mechanic.
